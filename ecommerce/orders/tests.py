...
class TestUnit(APITestCase):
    def setUp(self):
        ...
        self.orderitem1 = mommy.make(
            OrderItem, order=self.order1, product=self.product1)

    @tag('delete_orderitem')
    def test_updating_orderitem(self):
        url = reverse('delete_order_item', kwargs={
                      'order_id': self.order1.id, "orderitem_id": self.orderitem1.id})
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        url = reverse('list_processing')
    ...
    def test_paying_order(self):
        url = reverse('paying_order', kwargs={
    ...
    @tag('empty_all_order_paid')
    def test_empty_paid_order(self):
        url = reverse('paid_list')
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 0) 
    ...
    def test_paid_order(self):
        url_pay = reverse('paying_order', kwargs={
                          'order_id': self.order1.id})
    ...
    def test_updating_orderitem(self):
        ...
        url = reverse('updating_order_item',
                      kwargs={'pk': self.order1.id})
        data = {"product": self.product2.id, "qty": 1}
        response = self.client.post(url, data=data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        print(response.data)
        self.assertEqual(response.data["product"], self.product2.id)
        self.assertEqual(response.data["qty"], 1)
