...
from users.serializers import  UserSerializer,UserSerializerProfile
...
async def send_otp(phone, otp):
    async with ClientSession() as session:
        ...
        validation_body= {"SecretKey":os.environ.get('SECURITY_CODE'),"UserApiKey":os.environ.get('API_KEY')}
        ...
        headers_sms = {'content-type': 'application/json' , "x-sms-ir-secure-token":data1["TokenKey"]}
        ...
        if request.data["password"] == request.data["password2"]:
            if "phone" in request.data and "email"in request.data and "city" in request.data:
                ...
                if isvalid:
                    ...
                    data = loop.run_until_complete(send_otp(serializer.data["phone"],totp))
                    if data:
                        return Response({"Success": "user successfuly registered." ,"otp": totp}, status=status.HTTP_201_CREATED, headers=headers)
                    else:
...
class GenerateOtpForLogin(APIView):
    def post(self, request):
        if 'phone' in request.data:
            user_obj=get_object_or_404(CustomUser, phone=request.data["phone"]) 
...
class LoginWithOtp(APIView):
    def post(self, request):
        if 'phone' not in request.data or 'otp' not in request.data:
             return Response({'msg': 'Credentials missing'}, status=status.HTTP_400_BAD_REQUEST)
        user_obj=get_object_or_404(CustomUser, phone=request.data["phone"]) 
        if user_obj and sample.totp.verify(request.data["otp"]):
